...
def make_env(game, state, seed=0):
    def _init():
        ...
        env = StreetFighterCustomWrapper_prior(env)
        env = Monitor(env)
        env.seed(seed)
        return env
    return _init
...
def main():
    ...
    model = PPO(
        ...
        tensorboard_log="logs_prior"
    )



    save_dir = "trained_models_prior"
    ...
    checkpoint_callback = CheckpointCallback(save_freq=checkpoint_interval, save_path=save_dir, name_prefix="ppo_ryu_prior")
    ...
    model.save(os.path.join(save_dir, "self_sf2_ryu_final_p.zip"))
